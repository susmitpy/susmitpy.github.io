...
export function LiveProjectCard({ name, description, link, imagePath, gifPath }: CardProps) {
    return (
        <div className="max-w-xs w-full">
            <div
                className={cn(
                    "group w-full cursor-pointer overflow-hidden relative card h-75 rounded-md shadow-xl mx-auto p-4",
            ...
            >
                ...
                <img
                    src={imagePath}
                    alt={name}
                    className="object-cover transition-opacity duration-500 group-hover:opacity-0 py-2"
                />
                ...
                <a href={link} target="blank" className="pt-2 flex items-center justify-center group-hover:opacity-0 transition-opacity duration-500">
                    <button className="px-4 py-2 bg-blue-500 text-white rounded flex items-center text-sm">
